...
namespace Valve.VR.InteractionSystem
{
	...
	public class Throwable : MonoBehaviour
	{
		...
		[Header("KtQ additions")]
		public bool magicalThrowy;
		public GameObject magicThrowyThing;
		public float magicThreshhold = 1;
		...
		private void OnDetachedFromHand( Hand hand )
		{
			...
			if (magicalThrowy && velocity.magnitude >= magicThreshhold) {
				GetComponentInChildren<Collider> ().enabled = false;
				GameObject magic = Instantiate (magicThrowyThing, this.transform.position, Quaternion.identity) as GameObject;
				...
				magic.GetComponent<Rigidbody> ().velocity = rb.velocity;
				Destroy (this.gameObject);
			} else {
				velocity*=velocity.magnitude * 0.5f;
				Vector3 r = transform.TransformPoint( rb.centerOfMass ) - position;
				rb.velocity = velocity + Vector3.Cross( angularVelocity, r );
				rb.angularVelocity = angularVelocity;
			}
