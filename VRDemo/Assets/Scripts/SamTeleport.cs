...
{
	//-------------------------------------------------------------------------
	public class SamTeleport : MonoBehaviour
	{
		...
		private Hand mapHand;
		private bool mapTeleport = false;
		public bool teleportMapHand = false;

		[Header( "Float" )]
		public bool Float = false;
		public float flightDistance = 50;
		private int floatState = 1;
		public float lastY;

		[Header( "Buttons" )]
		public bool UseButtons = true;
		private VRButton pointedAtButton;
		...
		private LaserPointer teleportPointer = null;
		...
		//-------------------------------------------------
		private static SamTeleport _instance;
		public static SamTeleport instance
		{
			get
			{
				if ( _instance == null )
				{
					_instance = GameObject.FindObjectOfType<SamTeleport>();
				}
		...
		void Start()
		{
			...
			teleportPointer = GetComponent<LaserPointer>();
			teleportPointer.traceLayerMask = traceLayerMask;

			CheckForSpawnPoint();

			lastPos = player.trackingOriginTransform.position; 
		...
		void Update()
		{
			if (floatState != -1 && lastPos != player.trackingOriginTransform.position)
				HitGround ();

			foreach (Hand hand in player.hands) {
				if (Float && hand.controller != null && hand.controller.GetPressDown (SteamVR_Controller.ButtonMask.Touchpad) && hand.controller.GetAxis(Valve.VR.EVRButtonId.k_EButton_SteamVR_Touchpad).y > .1f) {
					player.trackingOriginTransform.position = player.trackingOriginTransform.position + new Vector3 (0, flightDistance * floatState, 0);
					floatState *= -1;
					lastY = player.trackingOriginTransform.position.y;
				}
				if (UseMap && hand.controller != null && hand.controller.GetPressDown (SteamVR_Controller.ButtonMask.Touchpad) && hand.controller.GetAxis(Valve.VR.EVRButtonId.k_EButton_SteamVR_Touchpad).y < -.1f) {
					if (mapHand != null && hand != mapHand) continue;
					GameObject map = hand.transform.Find (mapName).gameObject;
					bool val = !map.activeSelf;
					if (val)
						mapHand = hand;
					else
						mapHand = null;
			...
			if (floatState == -1 && player.trackingOriginTransform.position.y != lastY) {
				floatState = 1;
			}
		...
		private void UpdatePointer()
		{
			...
			RaycastHit hitInfo;
			teleportPointer.DrawLine (out hitInfo, pointerHand.transform);
			...
			if ( hitTeleportMarker != null ) //Hit a teleport marker
			{
				...
				{
					teleportPointer.SetColor( pointerLockedColor );
				...
				{
					teleportPointer.SetColor( pointerValidColor );
				...
				if (pointedAtButton != null)
					pointedAtButton.HoverLeave.Invoke ();
			...
			{
				if (UseMap && hitInfo.collider != null && hitInfo.collider.gameObject.tag == "Map") {
					destinationReticleTransform.gameObject.SetActive (false);
					...
					if (hitSomething) {
						...
						pointerEnd = teleportPointer.EndPos ();
					}
				...
				} else if (UseButtons && hitInfo.collider != null && hitInfo.collider.gameObject.tag == "Button") {
					destinationReticleTransform.gameObject.SetActive (false);
					offsetReticleTransform.gameObject.SetActive (false);
					invalidReticleTransform.gameObject.SetActive (false);
					...
					pointedAtButton = hitInfo.collider.GetComponentInParent<VRButton>();
					pointedAtButton.HoverEnter.Invoke ();
				}
				else {
					destinationReticleTransform.gameObject.SetActive (false);
					offsetReticleTransform.gameObject.SetActive (false);

					teleportPointer.SetColor (pointerInvalidColor);
		...
		private void HidePointer()
		{
			...
			teleportPointer.DestroyLine();

			foreach ( TeleportMarkerBase teleportMarker in teleportMarkers )
			{
				if ( teleportMarker != null && teleportMarker.markerActive && teleportMarker.gameObject != null && teleportMarker.GetType() != typeof(TeleportTerrain))
				{
		...
		private void TryTeleportPlayer()
		{
			if ( visible && !teleporting )
			{
				...
				if (UseMap && mapTeleport) {
					player.transform.position = mapHand.transform.Find (mapName).InverseTransformPoint (pointedAtPosition);
					HitGround ();
					mapTeleport = false;
				}
				if (UseButtons && pointedAtButton != null) {
					pointedAtButton.Click.Invoke ();
				}
			}
		}
		...
		public void HitGround() {
			...
			Ray raycast = new Ray (player.trackingOriginTransform.position + new Vector3(0, 200, 0), -player.trackingOriginTransform.up);
			bool bHit = Physics.Raycast (raycast, out groundHit);
			Vector3 normalToUse;
			float angle;
			Quaternion playerRotation;

			if (bHit) {
				player.trackingOriginTransform.position = groundHit.point;
					/*angle = Vector3.Angle(player.trackingOriginTransform.position, groundHit.normal);
					playerRotation = Quaternion.LookRotation (groundHit.normal);
					angle = -(playerRotation.eulerAngles.x + 90);
					player.trackingOriginTransform.rotation = Quaternion.AngleAxis(angle, player.trackingOriginTransform.right);*/
