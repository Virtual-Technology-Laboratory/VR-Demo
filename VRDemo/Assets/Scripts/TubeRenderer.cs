...
public class TubeRenderer : MonoBehaviour
{
	...
	void Reset()
	{
		...
		vertices.Clear ();
		vertices.Add (new TubeVertex (Vector3.zero, 1.0f, Color.white)); 
		vertices.Add (new TubeVertex (new Vector3 (1, 0, 0), 1.0f, Color.white));
	}

	public void AddPoint(Vector3 newPos, float r) {
		Vector3 localPos = transform.InverseTransformPoint (newPos);
		vertices.Add(new TubeVertex(localPos, r, Color.white));
		Draw ();
	}
	...
	void Start()
	{
		...
		vertices.Clear ();
		vertices.Add (new TubeVertex (Vector3.zero, 0f, Color.white)); 
	}

	/*void LateUpdate()
	{
		if (null == vertices || vertices.Count <= 1)
		{
			return;
		}
	...
	void SetPoints(Vector3[] points, float radius, Color col)
	{
		...
		vertices = new List<TubeVertex> (points.Length + 2);
		...
		vertices[vertices.Count - 1] = new TubeVertex(v1offset + points[points.Length - 1], 0.0f, col);
